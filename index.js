...
const firebaseConfig = {
  ...
  window.onload = async function() {
    ...
    if(current_page=="dashboard"){
      ...
      if(user){
      let getAgent=await getfirebaseItems("groups","creator","==",user.uid)
      let getusers=await getItemsonseveralconditions("groups","creator","==",user.uid,"members","array-contains",user.uid)
      }
    }
...
async function getItemsonseveralconditions(collectionName,field1,operator1,valueOne,field2, operator2,valuetwo) {
  ...
  const q = query(
      colRef,
      where(field1, operator1, valueOne),
      where(field2, operator2, valuetwo) // Check that another field is missing or null
  );
